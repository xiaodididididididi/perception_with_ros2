...
{
    PerceptionProcess::PerceptionProcess() : Node("perception_process_main")
    {
        RCLCPP_INFO(this->get_logger(), "perception_process created");
        process_config_ = std::make_unique<ConfigReader>();
        process_config_->read_perception_process_config();
    ...
    bool PerceptionProcess::process()
    {
        ...
        if (!perception_init())
        {
            RCLCPP_ERROR(this->get_logger(), "perception init failed!");
            return false;
        }

        timer_ = this->create_wall_timer(0.1s, std::bind(&PerceptionProcess::perception_callback, this));
        return true;
    }

    bool PerceptionProcess::perception_init()
    {
        vehicle_spawn(car_);
    ...
    void PerceptionProcess::vehicle_spawn(const std::shared_ptr<VehicleBase> &vehicle)
    {
    ...
    void PerceptionProcess::get_location(const std::shared_ptr<VehicleBase> &vehicle)
    {
        try
        {
            PoseStamped point;
    ...
    template <typename T>
    bool PerceptionProcess::connet_server(const T &client)
    {
    ...
    bool PerceptionProcess::map_request()
    {
    ...
    bool PerceptionProcess::global_path_request()
    {
    ...
    void PerceptionProcess::perception_callback()
    {
        ...
        const double perception_total_time = end_time.seconds() - start_time.seconds();
        RCLCPP_INFO(this->get_logger(), "perception total time: %fms\n", perception_total_time * 1000);

        if (perception_total_time > 1.0)
        {
            RCLCPP_ERROR(this->get_logger(), "perception time too long!");
            rclcpp::shutdown();
        }
    }
}